...
| --- | ---: | ---: |
| 样本总数 | 135 | 100% |
| DeepSeek 有输出（`deepseek_output.md`） | 135 | 100.0% |
| **Maven 通过**（`returncode=0`） | **94** | **69.6%** |
| 编译失败（`COMPILATION ERROR`） | 26 | 19.3% |
| 测试失败（已编译，`Tests run` 失败） | 8 | 5.9% |
| 合并失败（无 `@Test` 可合并） | 2 | 1.5% |
| 其它 Maven 非 0 | 5 | 3.7% |

## 结论摘要

- 在「A 底本 + 模型只改 `@Test` + 从 B 补 import/一层嵌套类」设定下，**约 70%** 样本能在 B 上跑通聚焦测试类。
- 主要失败形态：**编译失败**（缺符号/import/深层嵌套类，约 26 条）> **测试失败**（逻辑/断言未对齐，约 8 条）> **合并失败**（模型未输出可解析 `@Test`，2 条：cand124、cand128）。
...
| cand006 | 失败 | 成功 | `MethodUtilsTest` 编译失败 | PR#1414 LANG-1778 fix by reversing order (#1414)；合并后 testCompile 失败。`artifacts/sample_runs/cand006_1e455838963a/` |
...
| cand011 | 失败 | 成功 | `ClassUtilsTest` 编译失败 | PR#1495 Bugfix/class utils to canonical name length check (#1495)；合并后 testCompile 失败。`artifacts/sample_runs/cand011_5713c6c42e43/` |
...
| cand014 | 失败 | 成功 | `ClassUtilsTest` Maven 非 0 | PR#1437 [LANG-1774] Improve handling of ClassUtils.getShortCanonical。`artifacts/sample_runs/cand014_61c2d8f84931/` |
| cand015 | 失败 | 成功 | `MethodUtilsTest` 测试失败（断言/运行） | PR#1289 [LANG-1754] Use getAllSuperclassesAndInterfaces() in getMatc；编译通过但测试失败。`artifacts/sample_runs/cand015_65970408668e/` |
| cand016 | 失败 | 成功 | `StringsTest` 测试失败（断言/运行） | Avoid JDK-8015417 in Strings.equals()；编译通过但测试失败。`artifacts/sample_runs/cand016_6e78b38f0555/` |
...
| cand018 | 失败 | 成功 | `ClassUtilsTest` 编译失败 | PR#1494 ClassUtils now throws `IllegalArgumentException` when array；合并后 testCompile 失败。`artifacts/sample_runs/cand018_7cc1ac1c30d9/` |
| cand019 | 失败 | 成功 | `RuntimeEnvironmentTest` 编译失败 | PR#1323 Improve container detection by mimicing systemd (#1323)；合并后 testCompile 失败。`artifacts/sample_runs/cand019_7e0d6dbd2a65/` |
...
| cand021 | 失败 | 成功 | `TypeUtilsTest` 编译失败 | PR#1549 [LANG-1700] Improve handling of parameterized types and vari；合并后 testCompile 失败。`artifacts/sample_runs/cand021_8f8cc04e6550/` |
| cand022 | 失败 | 成功 | `ConversionTest` 测试失败（断言/运行） | Conversion.hexTo*() methods now throw IllegalArgumentExcepti；编译通过但测试失败。`artifacts/sample_runs/cand022_929df5d5c00a/` |
...
| cand026 | 失败 | 成功 | `DateUtilsTest` 编译失败 | PR#1609 LANG-771 Fix DateUtils.ceiling increment on exact boundary (；合并后 testCompile 失败。`artifacts/sample_runs/cand026_a9a0dd8a72c5/` |
...
| cand028 | 失败 | 成功 | `ClassUtilsTest` Maven 非 0 | PR#1577 Fix ClassNotFoundException message variable (#1577)。`artifacts/sample_runs/cand028_b62244517285/` |
...
| cand030 | 失败 | 成功 | `ArchUtilsTest` 编译失败 | PR#1625 Add "ppc64le" to ArchUtils (#1625)；合并后 testCompile 失败。`artifacts/sample_runs/cand030_bb675e1127ad/` |
| cand031 | 失败 | 成功 | `TypeUtilsTest` Maven 非 0 | PR#1548 [LANG-1749] Add stricter type checks for  parameterized type。`artifacts/sample_runs/cand031_bcad40a502cd/` |
...
| cand033 | 失败 | 成功 | `StopWatchTest` 编译失败 | PR#1473 LANG-1504 - Adding labels to split StopWatch feature (#1473)；合并后 testCompile 失败。`artifacts/sample_runs/cand033_ce2ba7b28260/` |
...
| cand042 | 失败 | 成功 | `DateUtilsTest` 测试失败（断言/运行） | PR#1385 Add org.apache.commons.lang3.time.DateUtils.toLocalDateTime(；编译通过但测试失败。`artifacts/sample_runs/cand042_0c6a5c10a7a0/` |
| cand043 | 失败 | 成功 | `SystemPropertiesTest` 编译失败 | Add SystemProperties.JAVA_SECURITY_KERBEROS_CONF；合并后 testCompile 失败。`artifacts/sample_runs/cand043_0dffedc822ce/` |
...
| cand046 | 失败 | 成功 | `LangCollectorsTest` 编译失败 | Add LangCollectors.collect(Collector, T...)；合并后 testCompile 失败。`artifacts/sample_runs/cand046_118652d768eb/` |
| cand047 | 失败 | 成功 | `LangCollectorsTest` 编译失败 | Make LangCollectors.collect(...) null-safe；合并后 testCompile 失败。`artifacts/sample_runs/cand047_17ba8b9da6a8/` |
...
| cand050 | 失败 | 成功 | `CalendarUtilsTest` 编译失败 | Add CalendarUtils.toLocalDateTime(Calendar)；合并后 testCompile 失败。`artifacts/sample_runs/cand050_328f2aedfb86/` |
...
| cand053 | 失败 | 成功 | `NumberUtilsTest` 测试失败（断言/运行） | [LANG-1729] NumberUtils.isParsable() returns true for full w；编译通过但测试失败。`artifacts/sample_runs/cand053_3bd162585900/` |
...
| cand057 | 失败 | 成功 | `TypeUtilsTest` 测试失败（断言/运行） | Fix StackOverflowError in TypeUtils.typeVariableToString(Typ；编译通过但测试失败。`artifacts/sample_runs/cand057_4e228a53509b/` |
| cand058 | 通过 | 成功 | `SystemUtilsTest` 通过 | Add SystemUtils.getJavaHomePath()。`artifacts/sample_runs/cand058_5035fdd788bc/` |
| cand059 | 失败 | 成功 | `ReflectionDiffBuilderTest` 编译失败 | Fix NullPointerException in ReflectionDiffBuilder.getExclude；合并后 testCompile 失败。`artifacts/sample_runs/cand059_5bcedccec3ed/` |
| cand060 | 失败 | 成功 | `RandomStringUtilsTest` 编译失败 | RandomStringUtils.random() methods may not return when asked；合并后 testCompile 失败。`artifacts/sample_runs/cand060_5d46a39e450f/` |
...
| cand075 | 失败 | 成功 | `MethodUtilsTest` 测试失败（断言/运行） | org.apache.commons.lang3.reflect.MethodUtils.getMethodObject；编译通过但测试失败。`artifacts/sample_runs/cand075_8a7b0da6fc78/` |
...
| cand083 | 失败 | 成功 | `SerializationUtilsTest` 编译失败 | [LANG-1759] SerializationUtils.clone(Object) throws ClassCas；合并后 testCompile 失败。`artifacts/sample_runs/cand083_a01471c196db/` |
...
| cand085 | 失败 | 成功 | `ClassUtilsTest` Maven 非 0 | org.apache.commons.lang3.ClassUtils.getCanonicalName(String)。`artifacts/sample_runs/cand085_a5f9a0cc22e4/` |
...
| cand088 | 失败 | 成功 | `EnumUtilsTest` 编译失败 | Add EnumUtils.getFirstEnum(Class<E>, int, ToIntFunction<E>,；合并后 testCompile 失败。`artifacts/sample_runs/cand088_b2e66fe50ba8/` |
...
| cand096 | 失败 | 成功 | `SystemPropertiesTest` 编译失败 | Add SystemProperties.getPath(String, Supplier<Path>)；合并后 testCompile 失败。`artifacts/sample_runs/cand096_da0583008020/` |
...
| cand110 | 失败 | 成功 | `MethodUtilsTest` 编译失败 | MethodUtils cannot find or invoke a public method on a publi；合并后 testCompile 失败。`artifacts/sample_runs/cand110_1d5ba7a1109b/` |
| cand111 | 失败 | 成功 | `BitFieldTest` 编译失败 | PR#1561 Add long support to `BitField` (#1561)；合并后 testCompile 失败。`artifacts/sample_runs/cand111_f2bdb0cb74a0/` |
...
| cand116 | 失败 | 成功 | `ValidateTest` Maven 非 0 | Add Validate.isTrue(boolean, Supplier<String>)。`artifacts/sample_runs/cand116_2188eb0e2e15/` |
...
| cand119 | 失败 | 成功 | `SystemPropertiesTest` 编译失败 | Add SystemProperties.getBoolean(Class, String, BooleanSuppli；合并后 testCompile 失败。`artifacts/sample_runs/cand119_8b27dec0340e/` |
| cand120 | 失败 | 成功 | `SystemPropertiesTest` 编译失败 | Add org.apache.commons.lang3.SystemProperties.isPropertySet(；合并后 testCompile 失败。`artifacts/sample_runs/cand120_9b383f3fadcc/` |
| cand121 | 失败 | 成功 | `RandomStringUtilsTest` 编译失败 | PR#1379 [LANG-1772] Restrict size of cache to prevent overflow error；合并后 testCompile 失败。`artifacts/sample_runs/cand121_c2260f094d78/` |
...
| cand124 | 失败 | 成功 | 未执行（合并失败） | ValueError: 补丁中未识别到任何 @Test 方法，且非完整测试类；Clean caches between tests。`artifacts/sample_runs/cand124_ef5de64ed438/` |
...
| cand128 | 失败 | 成功 | 未执行（合并失败） | ValueError: 补丁中未识别到任何 @Test 方法，且非完整测试类；Normalize parameter names。`artifacts/sample_runs/cand128_a511a163b579/` |
| cand129 | 通过 | 成功 | `ConsumersTest` 通过 | Flip args on new API。`artifacts/sample_runs/cand129_4c1cbdbb8002/` |
| cand130 | 失败 | 成功 | `FailableTest` 编译失败 | PR#1435 [LANG-1784] Add Functions|Failable methods for null-safe map；合并后 testCompile 失败。`artifacts/sample_runs/cand130_c49ed131f71d/` |
| cand131 | 失败 | 成功 | `RandomStringUtilsTest` 测试失败（断言/运行） | PR#1250 Feature/randomutils secure() and insecure() (#1250)；编译通过但测试失败。`artifacts/sample_runs/cand131_c95ee37dffd4/` |
