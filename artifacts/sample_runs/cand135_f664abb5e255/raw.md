...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 1723, 'completion_tokens': 725, 'total_tokens': 2448, 'prompt_tokens_details': {'cached_tokens': 256}, 'prompt_cache_hit_tokens': 256, 'prompt_cache_miss_tokens': 1467}`

---

- 修改 `testGetJavaVersion` 方法：新增 `assertEquals(JavaVersion.JAVA_24, JavaVersion.get("24"));` 断言，并将 `assertEquals(JavaVersion.JAVA_RECENT, JavaVersion.get("24"), "Unhandled");` 改为 `assertEquals(JavaVersion.JAVA_RECENT, JavaVersion.get("25"), "Unhandled");`，以匹配生产代码中新增的 `JAVA_24` 枚举值和 `get("24")` 的映射。
- 无需调整 import。
