...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 39175, 'completion_tokens': 2575, 'total_tokens': 41750, 'prompt_tokens_details': {'cached_tokens': 384}, 'completion_tokens_details': {'reasoning_tokens': 1282}, 'prompt_cache_hit_tokens': 384, 'prompt_cache_miss_tokens': 38791}`

---

- 新增 `testIsSorted`、`testIsSortedComparator`、`testIsSortedNullComparator`、`testIsSortedInt`、`testIsSortedFloat`、`testIsSortedLong`、`testIsSortedDouble`、`testIsSortedChar`、`testIsSortedByte`、`testIsSortedShort`、`testIsSortedBool` 共 11 个测试方法，覆盖所有新增的 `ArrayUtils.isSorted` 重载。
- 调整 import：将原分散的 `import static org.junit.Assert.*` 改为一条通配导入 `import static org.junit.Assert.*;`，并新增 `import java.util.Comparator;` 以支持 `Comparator` 参数测试。
