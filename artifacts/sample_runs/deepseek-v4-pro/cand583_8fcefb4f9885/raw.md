...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 3000, 'completion_tokens': 3092, 'total_tokens': 6092, 'prompt_tokens_details': {'cached_tokens': 384}, 'completion_tokens_details': {'reasoning_tokens': 1624}, 'prompt_cache_hit_tokens': 384, 'prompt_cache_miss_tokens': 2616}`

---

- 修改类 `StreamsTest` 声明为 `public class StreamsTest`，以匹配 B 上的测试类签名。
- 修改 `testSimpleStreamMap` 方法：添加 `public` 修复访问修饰符。
- 修改 `testSimpleStreamMapFailing` 方法：添加 `public`。
- 修改 `testSimpleStreamForEach` 方法：添加 `public`。
- 修改 `testToArray` 方法：添加 `public`。
- 修改 `simpleStreamForEachFailing` 方法：将 `@TestFactory` 方法的访问修饰符改为 `public`。
- 修改 `testSimpleStreamFilter` 方法：添加 `public`。
- 修改 `simpleStreamFilterFailing` 方法：将 `@TestFactory` 方法的访问修饰符改为 `public`。
- 不需要导入调整，生产代码仅添加 `@Deprecated`，未引入新类型。
