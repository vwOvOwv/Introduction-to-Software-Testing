...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 1793, 'completion_tokens': 2177, 'total_tokens': 3970, 'prompt_tokens_details': {'cached_tokens': 384}, 'completion_tokens_details': {'reasoning_tokens': 1522}, 'prompt_cache_hit_tokens': 384, 'prompt_cache_miss_tokens': 1409}`

---

- 需要在 `testGetJavaVersion` 方法中新增对 `JAVA_19`、`JAVA_20`、`JAVA_21` 的静态导入，因为生产代码新增了这三个枚举常量。
- 修改 `testGetJavaVersion` 方法：添加对 `"19"`、`"20"`、`"21"` 的断言，并将原来断言 `JAVA_RECENT` 的版本从 `"19"` 改为 `"22"`，使其符合 B 上未定义版本的处理逻辑。
