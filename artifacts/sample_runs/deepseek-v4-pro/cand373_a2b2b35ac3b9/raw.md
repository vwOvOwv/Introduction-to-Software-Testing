...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 29541, 'completion_tokens': 4643, 'total_tokens': 34184, 'prompt_tokens_details': {'cached_tokens': 384}, 'completion_tokens_details': {'reasoning_tokens': 947}, 'prompt_cache_hit_tokens': 384, 'prompt_cache_miss_tokens': 29157}`

---

1. 修改 `compareIsNumberWithCreateNumber` 方法：添加 `@SuppressWarnings("deprecation")`，将内部调用从 `NumberUtils.isCreatable` 改为 `NumberUtils.isNumber`，并调整断言消息中的方法名。
2. 修改 `testCreateNumberMagnitude` 方法：在原方法体中追加非零小数值与带小数字节的断言，以验证 `isZero` 逻辑。
3. 新增 `testCreateNumberZero` 测试方法：覆盖各种零值（含整数零、浮点零、指数、类型后缀）的 `createNumber` 行为。
4. 新增私有的静态辅助方法 `assertCreateNumberZero`，用于验证正零、带 `+` 号的正零以及负零。
5. 修改 `testIsCreatable` 方法：增加若干 LANG-1646 相关的 `compareIsCreatableWithCreateNumber` 调用。
6. 修改 `testIsNumber` 方法：增加若干 LANG-1646 相关的 `compareIsNumberWithCreateNumber` 调用。
7. 无需调整 import，现有 `import` 区块已满足所有新增类型需求。
