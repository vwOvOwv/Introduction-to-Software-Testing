...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 3146, 'completion_tokens': 1934, 'total_tokens': 5080, 'prompt_tokens_details': {'cached_tokens': 384}, 'completion_tokens_details': {'reasoning_tokens': 599}, 'prompt_cache_hit_tokens': 384, 'prompt_cache_miss_tokens': 2762}`

---

- 修改 @Test 方法 `testInvokeMethod`：将方法体内的 `new ImmutablePair<String, Object[]>` 全部替换为 `new ImmutablePair<>`，以匹配生产代码中的 diamond 语法习惯，并保证编译通过。
- 修改 @Test 方法 `testInvokeStaticMethod`：同样将 `new ImmutablePair<String, Object[]>` 替换为 `new ImmutablePair<>`。
