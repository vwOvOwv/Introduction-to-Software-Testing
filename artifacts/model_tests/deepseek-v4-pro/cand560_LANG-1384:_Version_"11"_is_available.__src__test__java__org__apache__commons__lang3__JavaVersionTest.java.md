...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 1345, 'completion_tokens': 1391, 'total_tokens': 2736, 'prompt_tokens_details': {'cached_tokens': 384}, 'completion_tokens_details': {'reasoning_tokens': 991}, 'prompt_cache_hit_tokens': 384, 'prompt_cache_miss_tokens': 961}`

---

- 修改 `testGetJavaVersion` 方法：新增对 `"11"` 返回 `JAVA_11` 的断言；将原 `get("11")` 断言改为 `get("12")`，以验证未知版本返回 `JAVA_RECENT`。无需调整 import（静态导入已涵盖新增枚举常量）。
