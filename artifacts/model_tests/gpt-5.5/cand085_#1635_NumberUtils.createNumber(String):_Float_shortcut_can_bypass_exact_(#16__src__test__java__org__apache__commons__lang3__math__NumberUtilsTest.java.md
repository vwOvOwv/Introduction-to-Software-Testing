...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 7720, 'completion_tokens': 2608, 'total_tokens': 10328}`

---

- 修改 `testCreateBigDecimal`：针对新增精度回归用例 `"0.100000001490116121"`，断言 `createBigDecimal` 保持 `BigDecimal` 精确值；不需要调整 import。
- 修改 `testCreateDouble`：针对同一字符串补充 `createDouble` 解析断言，以覆盖生产代码中 Float/Double 判定变化；不需要调整 import。
- 修改 `testCreateNumber`：新增断言 `"0.100000001490116121"` 应返回 `BigDecimal` 而不是 `Float`，对应 `createNumber` 中改为严格比较 `Float.toString()` 与 `Double.toString()`；不需要调整 import。
