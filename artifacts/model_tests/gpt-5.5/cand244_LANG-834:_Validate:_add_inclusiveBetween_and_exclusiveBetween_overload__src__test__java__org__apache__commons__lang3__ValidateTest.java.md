...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 4562, 'completion_tokens': 2254, 'total_tokens': 6816}`

---

- 修改 `testInclusiveBetween`：生产代码新增 `long`/`double` 原始类型重载后，将对象 `Comparable` 场景保留为 `String`，避免与新增原始类型测试混在一起；无需调整 import。
- 修改 `testInclusiveBetween_withMessage`：补充 `String` 版本自定义错误消息断言，并将原始类型覆盖拆分到新测试；无需调整 import。
- 新增 `testInclusiveBetweenLong`、`testInclusiveBetweenLong_withMessage`：覆盖新增 `inclusiveBetween(long, long, long[, String])` 重载；无需调整 import。
- 新增 `testInclusiveBetweenDouble`、`testInclusiveBetweenDouble_withMessage`：覆盖新增 `inclusiveBetween(double, double, double[, String])` 重载；无需调整 import。
- 修改 `testExclusiveBetween`：生产代码新增 `long`/`double` 原始类型重载后，将对象 `Comparable` 场景保留为 `String`；无需调整 import。
- 修改 `testExclusiveBetween_withMessage`：补充 `String` 版本自定义错误消息断言，并将原始类型覆盖拆分到新测试；无需调整 import。
- 新增 `testExclusiveBetweenLong`、`testExclusiveBetweenLong_withMessage`：覆盖新增 `exclusiveBetween(long, long, long[, String])` 重载；无需调整 import。
- 新增 `testExclusiveBetweenDouble`、`testExclusiveBetweenDouble_withMessage`：覆盖新增 `exclusiveBetween(double, double, double[, String])` 重载；无需调整 import。
