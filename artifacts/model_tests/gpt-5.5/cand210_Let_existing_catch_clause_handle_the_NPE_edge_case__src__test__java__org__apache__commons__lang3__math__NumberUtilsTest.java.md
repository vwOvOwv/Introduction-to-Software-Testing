...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 4649, 'completion_tokens': 2900, 'total_tokens': 7549}`

---

- 修改 `testStringToDoubleStringD`：针对 `toDouble(String, double)` 现在通过捕获 `RuntimeException` 处理 `null` 和空字符串，补充默认值断言；无需调整 import。
- 修改 `testToByteStringI`：针对 `toByte(String, byte)` 同样补充空字符串与 `null` 返回默认值的断言；无需调整 import。
- 修改 `testToFloatStringF`：针对 `toFloat(String, float)` 补充空字符串与 `null` 返回默认值的断言；无需调整 import。
- 修改 `testToIntStringI`：针对 `toInt(String, int)` 补充空字符串与 `null` 返回默认值的断言；无需调整 import。
- 修改 `testToLongStringL`：针对 `toLong(String, long)` 补充空字符串与 `null` 返回默认值的断言；无需调整 import。
- 修改 `testToShortStringI`：针对 `toShort(String, short)` 补充空字符串与 `null` 返回默认值的断言；无需调整 import。
- 修改 `testToScaledBigDecimalFloatIRM`、`testToScaledBigDecimalString`、`testToScaledBigDecimalStringIRM`：仅同步测试 diff 中的断言换行缩进，不改变断言语义；无需调整 import。

```java
    @Test
    public void testStringToDoubleStringD() {
        assertEquals(NumberUtils.toDouble("123.456", 5.1d), 123.456d, "toDouble(String, int) 1 failed");
        assertEquals(NumberUtils.toDouble("123.45", 5.1d), 123.45d, "toDouble(String, int) 2 failed");
        assertEquals(NumberUtils.toDouble("-123.45", 5.1d), -123.45d, "toDouble(String, int) 3 failed");
    ...
    public void testToFloatStringF() {
        assertEquals(1.2345f, NumberUtils.toFloat("1.2345", 5.0f), "toFloat(String, int) 1 failed");
