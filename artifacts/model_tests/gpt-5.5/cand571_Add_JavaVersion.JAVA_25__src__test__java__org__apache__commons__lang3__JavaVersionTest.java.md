...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 2072, 'completion_tokens': 1000, 'total_tokens': 3072}`

---

- 修改 `testAtLeast`：仅按测试 diff 增加 `// 0.x` 注释以保持与新版测试一致；不需要调整 import。
- 修改 `testGetJavaVersion`：生产代码新增 `JAVA_25` 和 `JavaVersion.get("25")` 分支，因此新增 `"25"` 的精确断言，并将未处理版本回退断言从 `"25"` 改为 `"26"`；不需要调整 import。
