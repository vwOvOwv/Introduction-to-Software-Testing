...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 2738, 'completion_tokens': 445, 'total_tokens': 3183}`

---

- 新增 `testToLocalDate`：生产代码新增 `CalendarUtils#toLocalDate()`，测试需覆盖 GMT 日历在默认时区为 GMT-5 时转换为 `LocalDate` 的行为。
- 需要调整 import：新增 `java.time.LocalDate`、`java.time.Month` 和 `org.junitpioneer.jupiter.DefaultTimeZone`。
