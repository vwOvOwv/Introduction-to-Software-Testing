...
- finish_reason: `stop`
- usage: `{'prompt_tokens': 2495, 'completion_tokens': 902, 'total_tokens': 3397}`

---

- 修改 `testComplexThreadGroups`：生产代码复用同一个 `NamePredicate` 查询多个同名线程组，因此测试新增两个同名 `thread_group_7__` 线程组及同名线程，并断言按线程名和线程组名可找到 2 个线程；无需调整 import。
...
```java
    ...
    public void testComplexThreadGroups() throws Exception {
        ...
        final List<ThreadGroup> threadGroups = Arrays.asList(threadGroup1, threadGroup2, threadGroup3, threadGroup4,
                threadGroup5, threadGroup6, threadGroup7, threadGroup7Doubled);
