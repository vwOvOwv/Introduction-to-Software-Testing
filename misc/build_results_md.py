...
def main() -> None:
    ...
    for i, c in enumerate(cands, 1):
        ...
        has_md = (d / "raw.md").is_file() or (d / "model_output.md").is_file()
...
- 流水线：`5_run.py` → `misc/update_tests.py`（模型）→ 合并进 A 底本并 **从 B 同步 import / 嵌套类** → `misc/run_single_maven_test.py` 在 **B** 上执行 `mvn -q -Dtest=<TestClass> test`。
...
| --- | ---: | ---: |
| 样本总数 | {n} | 100% |
| 模型有输出（`raw.md` / `model_output.md`） | {n_deepseek} | {n_deepseek / n * 100:.1f}% |
...
- 单条：`artifacts/sample_runs/candNNN_<B前12位>/`（`raw.md`、`generated.java`、`run_report.json`）
